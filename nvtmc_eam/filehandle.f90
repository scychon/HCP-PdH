...
contains
        ...
        do iarg=1,narg
          ...
          select case(adjustl(str))
            ...
            !Treat the second arg of a serie
              ...
              elseif(bLookForRval)then
                read(str,*) rHPerPd
                write(*,*) 'rHPerPd : ',rHPerPd
                bLookForRval=.FALSE.
              elseif(bExcludeSurfPd)then
                write(*,*) 'Surface Pd Atoms are now excluded'
              else
    ...
    subroutine readxyz
        ...
        do iAtom=1,nPdAtoms
            ...
            if (bUsePdGroup) then
                read(7,*) str,PdPos(:,iAtom),PdGroup(iAtom)
            else
                read(7,*) str,PdPos(:,iAtom)
            endif

            rCOM(:) = rCOM(:) + PdPos(:,iAtom)
            !write(*,*) PdPos(:,iAtom)
            idxPdInAtom(iAtom) = iAtom
        enddo
        rCOM(:) = rCOM(:)/(1.d0*nPdAtoms)
        !read(7, '(A)') line
        close(7)
        
    end subroutine readxyz

    ! Subroutine to get command arguments and assign input and output file name for Hanalysis
    subroutine getargsAnal ( aStrArgs )
        use variables
        implicit none
        ...
        integer :: narg,iarg
        logical::bLookForInp=.FALSE.
        logical::bLookForSkip=.FALSE.
        logical::bLookForStride=.FALSE.
        logical::bLookForTrajfile=.FALSE.
        logical::bFileExist
        character(len=256)         :: str, line
        integer::nskip
          ...
          select case(adjustl(str))
            ...
            !Treat the second arg of a serie
              if(bLookForInp)then
                strInFile=adjustl(str) !assign a value to pedfile
                bLookForInp=.FALSE. !put the logical variable to its initial value
              elseif(bLookForTrajfile)then
                strTrajFile=adjustl(str) !assign a value to pedfile
                write(*,*) 'strTrajFile : ',strTrajFile
                bLookForTrajfile=.FALSE.
              elseif(bLookForStride)then
                read(str,*) nStride
                bLookForStride=.FALSE.
              elseif(bLookForSkip)then
                read(str,*) nSkipFrame
                bLookForSkip=.FALSE.
              else
                write(*,*)"Option ",adjustl(str),"unknown"
        ...
        if(strInFile .eq. "") then
          write(*,*) 'input file has not set'
          write(*,*) 'usage : anal_htraj -f param.dat -traj traj.xyz -stride 10 -skip 100 -surf -grp'
          stop
        endif
