...
def gen_ener(pdpos,hpos):
    ...
    for jj in range(len(pdpos)):
      pdval = pdpos[jj]
      for kk in range(jj+1,len(pdpos)):
        pd2val = pdpos[kk]
        drpd = np.linalg.norm(pdval-pd2val)
        if drpd < 5.35 and drpd > 0.1:
            rhopd[jj]+=rhoa_pd(drpd)
            rhopd[kk]+=rhoa_pd(drpd)
            phi_pdpd += pi_pd(drpd)
      for kk in range(len(hpos)):
        hval = hpos[kk]
        drh = np.linalg.norm(pdval-hval)
        if drh < 5.35 and drh > 0.1:
            rhoh[kk]+=rhoa_pd(drh)
            phi_pdh += pi_pdh(drh)
            rhopd[jj]+=rhoa_h(drh)
      fpdval += fpd(rhopd[jj])
    for jj in range(len(hpos)):
      hval = hpos[jj]
      for kk in range(jj+1,len(hpos)):
        h2val = hpos[kk]
        drh = np.linalg.norm(hval-h2val)
        if drh < 5.35 and drh > 0.1:
            rhoh[jj]+=rhoa_h(drh)
            rhoh[kk]+=rhoa_h(drh)
            phi_pdh += pi_pdh(drh)
...
def comp_ener(pdpos,hpos):
    return np.asarray(gen_ener(pdpos,hpos)[0:8])-np.asarray(gen_ener(pdpos,[])[0:8])
...
def fcc_oct(aval):
    pdpos = []
    hpos = []
    hpos.append(np.asarray((0,0,0)))  # place H at the origin
    pdpos.append(np.asarray((1,0,0))*aval/2)
    pdpos.append(np.asarray((0,1,0))*aval/2)
    pdpos.append(np.asarray((0,0,1))*aval/2)
    pdpos.append(np.asarray((-1,0,0))*aval/2)
    pdpos.append(np.asarray((0,-1,0))*aval/2)
    pdpos.append(np.asarray((0,0,-1))*aval/2)
    return (pdpos,hpos)

def fcc_oct_second(aval):
    pdpos,hpos = fcc_oct(aval)
    pdpos.append(np.asarray((1,1,1))*aval/2)
    pdpos.append(np.asarray((-1,-1,-1))*aval/2)
    return (pdpos,hpos)

pdpos,hpos = fcc_oct(4.104)
comp_ener(pdpos,hpos)

pdpos,hpos = fcc_oct_second(4.104)
comp_ener(pdpos,hpos)

def fcc_tetra(aval):
    pdpos = []
    hpos = []
    hpos.append(np.asarray((1,1,1))*aval/4)  # place H at the tetrahedral center
    pdpos.append(np.asarray((1,0,0))*aval/2)
    pdpos.append(np.asarray((0,1,0))*aval/2)
    pdpos.append(np.asarray((0,0,1))*aval/2)
    pdpos.append(np.asarray((1,1,1))*aval/2)
...
def fcc_tetra_second(aval):
    pdpos,hpos = fcc_tetra(aval)
    pdpos.append(np.asarray((-1,0,0))*aval/2)
    pdpos.append(np.asarray((0,-1,0))*aval/2)
    pdpos.append(np.asarray((0,0,-1))*aval/2)
    return (pdpos,hpos)

pdpos,hpos = fcc_tetra(4.104)
comp_ener(pdpos,hpos)

pdpos,hpos = fcc_tetra_second(4.104)
comp_ener(pdpos,hpos)
...
for i in range(len(xlist)):
    ...
    listhcpoc[i] = ener_hcp_oct(xlist[i]/np.sqrt(2),covera_hcp*xlist[i]/np.sqrt(2),fhval)[0:8]
    listhcpte[i] = ener_hcp_tetra(xlist[i]/np.sqrt(2),covera_hcp*xlist[i]/np.sqrt(2),fhval)[0:8]

plt.clf()
plt.plot(xlist/np.sqrt(2),listfccoc[:,0]/4,'-^',color='b',markevery=5,label='FCC_OC')
plt.plot(xlist/np.sqrt(2),listfccte[:,0]/4,'--^',color='b',markerfacecolor="None",markevery=5,label='FCC_TE')
plt.plot(xlist/np.sqrt(2),listhcpoc[:,0]/2,'-s',color='r',markevery=5,label='HCP_OC')
plt.plot(xlist/np.sqrt(2),listhcpte[:,0]/2,'--s',color='r',markerfacecolor="None",markevery=5,label='HCP_TE')
plt.legend()
plt.xlabel(r'$a (\AA)$')
...
plt.clf()
plt.plot(xlist/np.sqrt(2),listfccoc[:,0]/4,'-^',color='b',label='FCC_OC')
plt.plot(xlist/np.sqrt(2),listfccte[:,0]/4,'--^',color='b',markerfacecolor="None",label='FCC_TE')
plt.plot(xlist/np.sqrt(2),listhcpoc[:,0]/2,'-s',color='r',label='HCP_OC')
plt.plot(xlist/np.sqrt(2),listhcpte[:,0]/2,'--s',color='r',markerfacecolor="None",label='HCP_TE')
plt.legend( prop=fontP,handletextpad=0.1)
plt.xlabel(r'$a (\AA)$')
plt.ylabel('E (eV/Pd atom)')
