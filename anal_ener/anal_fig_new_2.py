...
for k in range(len(listh)):
  ...
  np.savetxt('energy_r_h'+listh[k]+'.txt',enerdata.T)
...
for i in range(11):
  for k in range(len(listh)):
    if (k==8 and i==0):
        ...
        phasedata.append([enersum[0,i,k,0]**(1./3)*30/rscale[0], float(listh[k]), 0])
    elif (k>5 and i==0):
        ...
        phasedata.append([enersum[0,i,k,0]**(1./3)*30/rscale[0], float(listh[k]), 1])
    elif (i>5 or k>5):
...
np.savetxt('phase.txt',phasedata)
