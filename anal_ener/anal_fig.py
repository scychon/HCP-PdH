...
strsymbol = ['--o','--s','--^']
for i in range(len(listptl)):
    plt.plot(listr, (htypesum[i,:,5])/htypesum[i,:,0],strsymbol[i],color=listcolor[i],label=listlabel[i])
...
strsymbol = ['-.o','-.s','-.^']
for i in range(len(listptl)):
    plt.plot(listr, (htypesum[i,:,3])/htypesum[i,:,0],strsymbol[i],color=listcolor[i],label=listlabel[i])
...
plt.legend( prop=fontP,handletextpad=0.1)
plt.legend(bbox_to_anchor=(0.1,0.95), loc="lower left",ncol=3,prop=fontP,handletextpad=0.1)
